 ...
 *
 */
public class DRASCDataAnalyzeForwardExpressionHandler implements DRAExpressionHandler{
	
	DRASCData draSCData;
	
	public DRASCDataAnalyzeForwardExpressionHandler(DRASCData draSCData){
	...
	public void ifElseExpressionHandler(IfElseExpression expression,
		...
		draSCData.analyzeForward(conditionList, currentClassInstance);

		draSCData.analyzeForward(thenExpression, currentClassInstance);
		draSCData.analyzeForward(elseExpression, currentClassInstance);
	...
	public void loopExpressionHandler(LoopExpression expression,
		...
		draSCData.analyzeForward(expression.getLoopBody(), currentClassInstance);
	...
	public void binaryExpressionHandler(BinaryExpression expression,
		...
		if(op.equals("=")){
			...
			draSCData.analyzeForward(tempListRight, currentClassInstance);
		
		}else{
			//e.g.: ==, <, +, *

			draSCData.analyzeForward(tempListLeft, currentClassInstance);
			draSCData.analyzeForward(tempListRight, currentClassInstance);
		}
	...
	public void functionCallExpressionHandler(
		...
		draSCData.analyzeForward( expression.getFunction().getBody(), currentClassInstance);
				...
				if(scci instanceof SCPortInstance){
					...
					if(scci.getPortSocket()==port){
						...
						for(SCKnownType knownType : ((SCPortInstance) scci).getChannels() ){
							...
							draSCData.analyzeForward(scfunction.getBody(), knownType);
						...
						for(SCClassInstance classInstancePort :((SCPortInstance) scci).getModuleInstances()){
							...
							draSCData.analyzeForward(scfunction.getBody(), classInstancePort);
	...
	public void SwitchCaseExpressionHandler(SwitchExpression expression,
		...
		for(Expression caseExpression: expression.getCases()){
			if(caseExpression instanceof CaseExpression){
				draSCData.analyzeForward(((CaseExpression) caseExpression).getBody(), currentClassInstance);
			}
		}
	}

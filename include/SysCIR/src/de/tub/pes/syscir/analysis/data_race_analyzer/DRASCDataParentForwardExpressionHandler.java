 ...
 *
 */
public class DRASCDataParentForwardExpressionHandler implements DRAExpressionHandler {
	
	private DRASCData draSCData;
	
	private Expression currentExpression;
	
	public DRASCDataParentForwardExpressionHandler(DRASCData draSCData){
	...
	private void analyzeSubListAndParentExpression(List<Expression> parentList, 
		...
		list = parentList.subList(begin, end);

		draSCData.analyzeForward(list, currentClassInstance, parentExpression);
	...
	public void ifElseExpressionHandler(IfElseExpression parentExpression,
		...
		}else if(elseExpression.contains(getCurrentExpression())){
			parentList = elseExpression;
		}else if(condition == getCurrentExpression()){
	...
	public void loopExpressionHandler(LoopExpression parentExpression,
		...
		if(draSCData.getAtomicBlock().checkDeeplyExpressionListForWaitStatement(list, currentClassInstance)){

			draSCData.analyzeForward(list, currentClassInstance);			

		}else{
			
			draSCData.analyzeForward(parentList, currentClassInstance);
	...
	public void binaryExpressionHandler(BinaryExpression expression,
		...
		if(expression.getRight()==currentExpression){
			...
			analyzeSubListAndParentExpression(list, expression, currentClassInstance);
	...
	public void sCVariableDeclarationExpressionHandler(
		...
		if(currentExpression==expression.getVariable()){
			...
			analyzeSubListAndParentExpression(expression.getInitialValues(), expression, currentClassInstance);
		}
	...
	public void functionCallExpressionHandler(
		...
		if(parentExpression.getParent() != null && parentExpression.getParent() instanceof AccessExpression){
			...
			if(left instanceof SCPortSCSocketExpression &&
				...
				System.out.println("Error: Did not find class instance of AccessExpression (DRASCData Parent Expression Forward Handler)"+" "+accessExpression);
				return;
	...
	public void SwitchCaseExpressionHandler(SwitchExpression expression,
		...
		if(currentExpression == expression.getSwitchExpression()){
			...
			analyzeSubListAndParentExpression(expression.getCases(),expression,currentClassInstance);
